...
class MatrixSlideshow
{
	...
	public function __construct()
	{
		...
		if( !is_dir(MTR_PLUGIN_UPLOADS_DIR) )
			$this->errors[] = __('Matrix Gallery: The uploads dir does not exists, please create "matrix" directory under "'.WP_CONTENT_DIR.' /uploads/"');
		if( is_dir(MTR_PLUGIN_UPLOADS_DIR) && !is_writable(MTR_PLUGIN_UPLOADS_DIR) )
			$this->errors[] = sprintf(__('Matrix Gallery: The upload dir "%s" is not writable, please set writte permissions'), MTR_PLUGIN_UPLOADS_DIR);
		if( !is_dir(MTR_PLUGIN_XML_DIR) )
			$this->errors[] = __('Matrix Gallery: The folder to store xml files does not exits, please create it and set write permissions');
		if( is_dir(MTR_PLUGIN_XML_DIR) && !is_writable(MTR_PLUGIN_XML_DIR) )
			$this->errors[] = sprintf(__('Matrix Gallery: The folder to store xml files "%s" is not writable, please set writte permissions'), MTR_PLUGIN_XML_DIR);
	...
	public function add_menus()
	{
		...
		add_submenu_page('matrix_menu', __('Matrix Slideshow Management'), __('Album Management'), 8, 'matrix_manage', 
...
insert xml code part
	...
	public function activate()
	{
		...
		$query[] = "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}mtr_misc(id int not null auto_increment, 
							ione int, 
							itwo int,
							ithree int,
							txt text,
		...
		$query[] = "INSERT INTO {$wpdb->prefix}mtr_misc (id, ione, itwo, ithree, txt) VALUES (1,1,1,1,'".$sec_word."')";					
		foreach($query as $q)
		{
			$wpdb->query($q);
		}
	...
	public function deactivate()
	{
		...
		//$wpdb->query($query);
		$query = "DROP TABLE {$wpdb->prefix}mtr_misc";
		$wpdb->query($query);
